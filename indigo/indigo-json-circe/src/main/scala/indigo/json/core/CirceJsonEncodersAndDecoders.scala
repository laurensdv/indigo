...
import indigo.shared.formats.TiledMapObjectPolygonPoint
...
object CirceJsonEncodersAndDecoders {
  ...
  implicit val decodeTiledMapObjectPolygonPoint: Decoder[TiledMapObjectPolygonPoint] =
    ...
    new Decoder[TiledLayer] {
      final def apply(c: HCursor): Decoder.Result[TiledLayer] =
        for {
          ...
          visible <- c.downField("visible").as[Boolean]
          objects <- c.downField("objects").as[Option[List[TiledMapObject]]]
        } yield TiledLayer(
          name,
          data,
          x,
